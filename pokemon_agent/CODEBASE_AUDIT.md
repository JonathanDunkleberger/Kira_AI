...
| Looping NPC — TRAVEL bumps a plain blocker on the only gap | `travel` chokepoint gauntlet | ✓ FIXED **LAYER A** (mark body tile in the unified `_blocked_npcs` set, route around, `no_route_npc_blocked` if none) — handles a genuine STATIONARY plain-NPC with a route around. **Live-verify pending** |
| **CUT-TREE mis-read as a plain NPC** at a sole exit (the "Slowbro-near-Cerulean-Mart wedge" — 2026-06-28 RE-DIAGNOSED) | `travel._npc_tiles` / Layer-A gauntlet | ❌ NOT A BUG IN THE NPC LOGIC — it's an HM GATE. The live Cerulean south exit is blocked by a **cuttable tree** (object-event gfx **95 = `GFX_CUT_TREE`**, source-confirmed) at **(26,32)**, the sole gap in the south fence; `_npc_tiles()` reads it as a body tile so Layer-A treats it as a "plain NPC" and sticky-blocks/routes-around forever. The earlier "two wanderer NPCs" call was an artifact of tracing from `misty_done` (player far → the tree object isn't spawned; only the 2 flanking NPCs load). **REAL fix = recognise gfx 95/97/92 as Cut/Strength/item obstacles (field_moves already detects them, source-cited) and surface "I need Cut" to the oracle — NOT permanent-block. Progression past it needs HM01 Cut (she does NOT know it yet) → the Bill→S.S.Ticket→S.S.Anne→Cut questline + `POKEMON_FIELD_MOVES` actuation (gated OFF/unverified).** See STATE §7.7. |
