...
# Separate WS manager + state store for stream-screen overlays.
...
async def ws_screens(ws: WebSocket):
    ...
    On connect: send all current overrides so reconnecting screens restore
    their override state without waiting for the next send.
    Then keep the connection alive waiting for client disconnect.
    """
