...
1. **BUILD AN EVENED-BENCH FIXTURE POSITIONED AT/PAST FUCHSIA (the clean unblock — do this instead of fighting the parity-sensitive Route-9 nav).** Take `surf_ready_kit` (AT Fuchsia, Surf+Strength, badge 6, but THIN bench L8-25) and grind its bench up to ~L28-32 with the PROVEN `recon_grind_bench.py` (Fuchsia has Route 15/18 grass), bank it as e.g. `fuchsia_evened_kit`. Then run the multi-gym solo look-ahead FORWARD from there: Fuchsia→(Safari already done→)Seafoam→Cinnabar→**Blaine (badge 7)**→**Giovanni (badge 8)**, both levers ON, and confirm the solo keeps firing + the bench evens toward 45/55 across the gym boundaries (the actual behavioral demo). This sidesteps the Route-9 boot-pocket entirely + gives the right team-shape.
