...
2. **QUESTION 2 (reach Koga with a LEVELED bench + clear it) — NOT ANSWERABLE in a 45-min budget.** She earned badge 4 and is grinding toward Koga, but the badge-4 → Silph Scope → Pokémon Tower → Poké Flute → Snorlax → Route 12/13 → Fuchsia chain is FAR past a single 45-min window. To answer Q2 you need a **MULTI-HOUR or RESUMABLE-CHECKPOINT run** (bank at badge 4, resume toward Koga), OR boot from a nearer fixture (a fresh badge-4 kit w/ FLASH+Flute done). RE-RUN a longer window: `POKEMON_ACE_BAIL=1 POKEMON_BENCH_TO_MILESTONE=1 POKEMON_PREP_DRY_RESET=1 LONGRUN_BATTLE_LOG=1 POKEMON_KEEPER_STATIC_ROUTE=1 POKEMON_KEEPER_ROUTER=1 ../.venv/Scripts/python.exe -u recon_longrun.py surge_done_kit.state 120 > /g/temp/longrun/ns4_koga.log 2>&1 &` (120-min), then grep `Soul Badge|badges=5|LOPSIDED-DBG|PREP RE-ARM|BATTLE-LOOP BREAKER|party=\[.venu`.
