...
## ✅ NIGHT-SHIFT #26 DONE (2026-07-12, night_shift.ps1 shift 26) — VERIFIED the NS#25 PP-heal lever (DECISIVE NEGATIVE: fires 0×, redundant with the ACE-DOWN guard) → held OFF; then BUILT + look-ahead-verified-SAFE the higher-leverage SELECTIVE-SOLO kill-XP lever (frontier #2) done SAFELY (suppresses a switch, never adds one — flag `POKEMON_SOLO_OVERLEVEL_GRIND` DEFAULT OFF), decision-verified 17/17, COMMITTED `7118157`. The solo look-ahead proved the lever FIRES + is SAFE (0 faint/churn/park, bench evens 28→31) but the RATE benefit was masked by low Route-10 wilds → **held OFF pending a clean rate proof on an ADEQUATE grind spot (VR2F)**. Two commits (`7118157` lever, `e46a6f2` docs). START HERE → the flip gate is a clean A/B on adequate wilds (frontier #1 below).
...
### ✅ THE SOLO LOOK-AHEAD RESULT (ns26_solo.log, koga_done_kit + SOLO ON — VERIFIED SAFE, rate INCONCLUSIVE, HELD OFF):
...
- **BENCH LEVELS + EVENS:** in ONE long solo stint the fielding cycled the WHOLE bench up together — slot 2(28)→5(28→31)→4(29→32)→3(30)→1(31→33) — bench floor ~28→~31, evening up (exactly the desired shape).
- **RATE INCONCLUSIVE (the confound worth capturing):** it took **~340 solo battles for ~5 levels** because Route-10 wilds are **L11-17, far below the L28-34 bench** → even FULL kill XP is small (~30 battles/level). So a clean "≥2× vs the ns25 participation baseline (floor 28→31 in 25 min)" could NOT be isolated — the bank throttling (a grind stint is ONE long roam tick → banks are sparse) + the low-XP-wild floor both mask the share-vs-solo delta. **KEY SYNTHESIS: the DOMINANT endgame bench throttle is grind-spot WILD-LEVEL adequacy (NS#5), not the participation share.** The solo lever is a real +XP but modest on low grass; it COMPOUNDS with an adequate spot — VR2F cave-grind (L36-46 wilds, ARMED `POKEMON_CAVE_GRIND` default ON) is where solo+adequate-wilds together would actually 2× a near-milestone bench climb.

### ⇒ NS#26 FRONTIER (priority order):
1. **GET A CLEAN RATE PROOF THEN FLIP the selective-solo — but measure it on an ADEQUATE grind spot, not Route 10.** The lever is decision-verified (17/17) + look-ahead-verified SAFE (fires, replaces the switch, 0 faint/churn/park, bench evens). The ONLY missing gate is a clean "climbs FASTER" number, which Route-10's L11-17 wilds mask. THE CLEAN MEASUREMENT: run TWO short arms to a FIXED bench-floor target and compare WALL-clock/battles — an A/B where the grind spot's wilds are near the bench level (so XP-per-kill isn't the floor). Best: a VR2F fixture (indigo_reach_g, `POKEMON_CAVE_GRIND=1`) where wilds L36-46 ~ a bench L36-43 → solo full-XP is a real ~2× over the share. If solo lands the bench-floor faster with no faint/churn → **flip `SOLO_OVERLEVEL_GRIND` default ON (battle_agent.py ~169) + commit.** (Env-tune `SOLO_OVERLEVEL_MARGIN` if a lower-XP wild ever risks a 2-turn kill → in-battle heal.)
2. **THE REAL FRESH-GO LEVELED-6 LEVER IS grind-spot ADEQUACY, and solo COMPOUNDS it.** The endgame bench can't reach ~L55 on L11-17 Route-10 grass at any XP-split. The armed VR2F cave-grind (`POKEMON_CAVE_GRIND` ON, NS#17/#21) is the adequate spot; the SELECTIVE-SOLO on top gives the bench the KILL XP there → the two together are the fresh-GO leveled-6 combo. VERIFY: a fresh badge-8→VR2F look-ahead with BOTH on, `recon_partydump` the bench at Indigo — does it arrive ~L46+ (vs VR2F-share's ~L43 cap, NS#22)? That's the number that gates whether the champion falls (needs Lapras ~L65-72 OR two L55+ attackers, NS#22 bracket).
3. **PP-heal stays OFF** (fires 0×, redundant with ACE-DOWN) — a filed safety net, not a bench lever.
4. **Load-share stays OFF** (NS#23) — building block for a fresh-GO team with two overlapping SE attackers.
5. ⚠️ **banked_LIVE is now a SOLO-ON run** (koga_done_kit, badge 6, bench climbing past L31). If the solo run is still alive at resume, reap it (`taskkill //F //IM python.exe //T`) or read banked_LIVE for the current solo-leveled bench. The koga_done_kit fixture is the clean baseline-comparable start.
