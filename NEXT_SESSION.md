...
## ⏳ NIGHT-SHIFT #26 IN FLIGHT (2026-07-12, night_shift.ps1 shift 26) — VERIFIED the NS#25 PP-heal lever (DECISIVE NEGATIVE: fires 0×, redundant with the ACE-DOWN guard) → held it OFF; then BUILT the higher-leverage SELECTIVE-SOLO kill-XP lever (frontier #2) SAFELY (suppresses a switch, never adds one — flag `POKEMON_SOLO_OVERLEVEL_GRIND` DEFAULT OFF), decision-verified 17/17, COMMITTED `7118157`. A solo look-ahead is IN FLIGHT (`/g/temp/longrun/ns26_solo.log`, koga_done_kit + SOLO ON). START HERE → read that log, and if the bench climbs FASTER with no faint/park/matchup-churn, FLIP the flag ON + commit.
...
### ⏳ THE SOLO LOOK-AHEAD (in flight — the flip gate):
`cd pokemon_agent && POKEMON_SOLO_OVERLEVEL_GRIND=1 POKEMON_GRIND_PP_HEAL=1 POKEMON_KEEPER_STATIC_ROUTE=1 POKEMON_KEEPER_ROUTER=1 LONGRUN_BATTLE_LOG=1 LONGRUN_BANK_EVERY_S=120 ../.venv/Scripts/python.exe -u recon_longrun.py koga_done_kit.state 90` → `/g/temp/longrun/ns26_solo.log`. Boot = koga_done_kit (badge 6, Venusaur L62 + bench L28-29 — the SAME start as the ns24/ns25 baseline, so it's apples-to-apples). AT RESUME grep: `SOLO-OVERLEVEL:|GRIND SWITCH: weak lead out|live-bank|GRIND-WEAK: floor crossed|MATCHUP SWITCH|blacked out`. THE QUESTIONS (vs the ns25 baseline floor 28→31 in 25 min): (1) does `SOLO-OVERLEVEL` FIRE (the bench mon out-levels the Route-10 wilds → solos)? (2) does the bench floor rise FASTER (target: meaningfully > 0.12 lvl/min — the ~2× kill-XP win)? (3) NO faint-thrash (the tight margin should prevent it — 0 `blacked out`)? (4) NO matchup-churn (0/low `MATCHUP SWITCH` during grind — the wedge-safety invariant holds live)? (5) NO park (she still marches forward)? **If clean → flip `SOLO_OVERLEVEL_GRIND` default ON (battle_agent.py ~169) + commit** — the bench-leveling lever is armed. If it faint-thrashes / churns → raise `SOLO_OVERLEVEL_MARGIN` (env) or keep OFF + document. ⚠️ watch-quality note: soloing means the weak mon takes the kill on-screen — verify it reads well (not slow/circly). NB per rule-15 the in-battle white-box actuation ideally wants an attended frame-grab before a live GO, but this lever REMOVES switch exposure so it's lower-risk than the switch-heavy path.

### ⇒ NS#26 FRONTIER (priority order):
1. **VERIFY + FLIP the selective-solo lever** (above) — the shift's build, the real bench-leveling win, verify-gated. This is the highest-leverage tractable lever for the fresh-GO leveled-6.
2. **USE the solo run (or a resume) to quantify the Indigo-arrival bench** — if the solo lever levels the bench faster, resume the koga_done_kit run toward badge 7→8→VR→Indigo and `recon_partydump` the arrival bank (the data that proves/sizes whether the leveled-6 actually arrives E4-ready). The champion falls to Lapras ~L65-72 OR two healthy L55+ coverage attackers (NS#22 bracket).
3. **PP-heal stays OFF** (verdict above) — it's a filed safety net, not a bench-leveling lever.
