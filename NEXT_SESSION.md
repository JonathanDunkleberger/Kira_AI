...
### ✅ ALSO BUILT SHIFT 14 — `victory_road.py` (the VR extraction, unwired/flag-OFF until smoke confirms; the NEXT gate on the credits chain):
The autonomous badge-8 dispatch (above) is COMMITTED + proven. Continued the climb: extracted `recon_victory.py` (942-line proven VR vehicle — already banked indigo_reach_g on the champion climb) into an in-loop `victory_road.VictoryRoad` + `run_strike(camp, log, dbg_dir)` on the LIVE campaign bridge (SAME giovanni_gym extraction pattern). Faithful port of the whiteout-tolerant map-dispatch loop: Viridian→Route 22 (Gary fight)→gate→Route 23→VR boulder floors (the hand-solved elevation-aware VR1F/2F/3F_PUZZLE constants verbatim)→R23 north→Indigo Plateau + heal. Returns `reached_indigo`/`battle_loss`/`stuck`. EQ teach = Phase 0, DEFAULT OFF (`POKEMON_TEACH_EQ`; recon NS12 proved it did net harm — Razor Leaf x2 carries VR). Parses + imports clean. **SMOKE-VERIFYING NOW** (`recon_victory_road_smoke.py`, from `giovanni_kit_g` badge-8 leveled fixture: booted Viridian→crossed to Route 22→Gary fight w/ MATCHUP SWITCH Venusaur→Lapras). NOT WIRED into the loop yet + NOT committed (build-then-smoke cadence, exactly how giovanni_gym was built shift 13 then wired shift 14).

### ⇒ SHIFT-14 FRONTIER (exact next actions, priority order):
1. **CONFIRM the VR smoke** (`/g/temp/longrun/ns14_vr_smoke.log`): grep `SMOKE PASS|reached_indigo|INDIGO PLATEAU|VICTORY ROAD CLEARED|abort LOUD|wedge`. SUCCESS = `SMOKE PASS` (res=reached_indigo, at Indigo (3,9)). Re-run: `VICTORY_ROAD_STATE=giovanni_kit_g /g/JonnyD/NeuroAI_Bot/.venv/Scripts/python.exe -u recon_victory_road_smoke.py` (⚠️ run from pokemon_agent/; use ABSOLUTE python path — cwd resets on backgrounded cmds). If a port bug surfaces (the puzzle sequences are proven, so any failure is an extraction slip — compare against recon_victory.py's closure line), fix + re-run. If it clears → COMMIT victory_road.py + the smoke.
2. **THEN WIRE the VR endgame dispatch** (the NEW wiring — VR is NOT a gym/gate-flag questline; there's no next_gym after badge 8 and no single VR-cleared flag). The clean approach: a PROACTIVE ENDGAME dispatch — when ALL 8 badges held (0x827 set) AND map != INDIGO (3,9) AND a `VICTORY_ROAD_ENABLED` flag is on, dispatch `victory_road.run_strike` (it drives its OWN road Viridian→Indigo, whiteout-tolerant). Hook it near the beat_gym/questline dispatch or as a proactive check in `_ensure_forward_questline` (like the giovanni gate but keyed on badges==8 + not-at-Indigo, NOT on next_gym — after badge 8 next_gym is None; goal reframes to "Take on the Elite Four"). Flag-gate OFF, verify a look-ahead from a badge-8 fixture drives Viridian→Indigo autonomously, then flip ON + commit. ⚠️ TEAM REALITY: a FRESH-chain badge-8 team is a solo L60 Venusaur + L8-25 bench — VR is survivable (Razor Leaf carries) but the E4 (L54-63 + champion) needs a bigger team; team-depth is the parallel carry-over, not this gate's problem.
3. **THEN the E4 vehicle** — `recon_e4.py` (682 lines: League-mart Full-Restore-first stock-up + the 5-room gauntlet Lorelei→Bruno→Agatha→Lance→Champion Gary → HALL OF FAME → CREDITS). SAME extract-and-hook: extract `e4_strike.run_strike(camp,log)` on the live bridge, dispatch when AT Indigo (3,9). The battle-brain is proven on the champion climb (23487e7 cracked the E4 wall: don't sleep-lock a 2x-SE foe; field the SE specialist reserve). Entering the Hall of Fame = CREDITS (auto-save, point of no return). ⚠️ needs a leveled team (the champion clear used L90/L72/L58); the fresh-chain thin team likely walls — team-depth is the gate before a fresh-GO credits roll.
