...
## ✅ NIGHT-SHIFT #16 DONE (2026-07-11, night_shift.ps1 shift 16) — killed the "Lapras has no Ice move" ROOT: grind-mon level-up moves now STICK. Every grind caller reorders the weak specialist to slot 0 but `_ensure_move_room` only ever ran on the PRE-reorder lead (the ace) — so a fielded bench mon leveling through a learnset threshold (Lapras→Ice Beam @L43, Kadabra's Psychic learns) hit the un-actuatable "Delete a move?" box and DECLINED the learn → reached the E4 without its signature coverage move. ONE commit (`e516266`), mode-side, canonical UNTOUCHED. START HERE → the binding wall is now purely LEVELING the bench (grind-spot adequacy at the endgame).
...
1. **CAVE STEP-ENCOUNTER GRINDING (the highest-payoff unbuilt capability, NS#14-flagged).** `grind()` needs GRASS tiles; VR/Cerulean Cave use step-encounters (wilds fire on walking, no grass). Teach the grind harness cave step-encounter pacing (wander walkable non-warp tiles, `avoid=doors`, so wilds fire on foot) — the SAME mechanic `catch_one`'s cave branch (`80789ab`) already uses for cave keeper-catching. VR wilds are L36-46 = ADEQUATE to level a L37-43 team to the E4 floor ON THE PATH (she crosses VR anyway). This is the cleanest unblock: level in the cave she's already walking through, and Lapras crosses L43 → Ice Beam (this shift's fix lands it).
