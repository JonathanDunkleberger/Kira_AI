...
### 🎯 THE NEW ROOT (fully diagnosed, the real find this shift): **the BENCH-TO-MS participation grind wears the ACE to a FAINT, which force-fields a moveless bench mon into an unwinnable-grass livelock.** Chain: (1) BENCH-TO-MS grind fires on Route 11 (`productive bite 8->13`); the participation-XP switch makes the **ace eat a FREE switch-in hit every wild** with NO inter-battle heal → over a long bench-grind the L36 Venusaur (+Diglett) wears down and **FAINTS**. (2) grind ends, "restoring the ace to lead" — but the ace is FAINTED → next wild auto-sends the first conscious mon = **Abra (only knows Teleport — ZERO damaging moves)**. (3) battle brain tries Abra→Spearow switch but the **white-box menu impostor fails it** → 3 stuck turns → flee → immediately re-encounter → **infinite livelock**; heal-excursion heads west to Vermilion CORRECTLY but every grass step re-encounters a 'stuck' battle → BATTLE-LOOP BREAKER → retry same leg forever; heal marked dead → head_to_gym no-ops. (Deep white-box switch wedge = the flagged E4-livelock-family, DON'T fix blind.)

### ✅ FIX BUILT + COMMITTED `2defbcd` — ACE-DOWN GUARD (mode-side, flag `POKEMON_ACE_BAIL` default ON, one-char revert; canonical UNTOUCHED): `grind()`'s FRAGILE (bench) path now HEALS the moment the ace dips to **≤34% HP** (`GRIND_ACE_BAIL_FRAC`, new helper `_ace_hp_frac` reusing `party_health()`) — while the ace is **still ALIVE to fight the escape battles out of the grass** — then returns `ace_healed` so roam re-grinds with a topped-up ace (one heal per tick, no same-call thrash). Prevents the faint that starts the whole cascade. **Fail-open** (a healthy ace one-shots wilds & never dips here; non-fragile ace-grind unchanged — ace-as-lead faint already handled by travel need_heal/battle_loss). `recon_bench_milestone_check` 12/12 + `recon_lopsided_grind_check` UNREGRESSED; compiles clean.

### ⏳ VERIFICATION RUN IN FLIGHT (the ship-gate — READ ITS TAIL AT RESUME): `G:/temp/longrun/ns11_acebail.log`, fresh surge_done_kit 45-min, `POKEMON_ACE_BAIL=1 POKEMON_BENCH_TO_MILESTONE=1 POKEMON_PREP_DRY_RESET=1 LONGRUN_BATTLE_LOG=1 POKEMON_KEEPER_STATIC_ROUTE=1 POKEMON_KEEPER_ROUTER=1` (launched ~13:07). **TWO questions, in order:** (1) **Does the Route-11 livelock stay dead?** grep `GRIND: ACE dinged to` (the guard firing + healing) and confirm NO `BATTLE-LOOP BREAKER` storm / NO ace at `L\d+.FAINTED` stranded in grass. (2) **THEN the original prep-dry question:** does she now **STOP on Route 13/14/15 and grind the bench** (PREP RE-ARM fires; LOPSIDED-DBG re-prints post-badge-4), arrive at Koga with a **leveled bench**, and **progress PAST the post-Koga Route-15 stall toward Saffron**? **AT RESUME grep:** `GRIND: ACE dinged|PREP RE-ARM|PREP STAND-DOWN|LOPSIDED-DBG|BENCH-TO-MS|Soul Badge|badges=5|badges=6|OUTCOME:|STALL|BATTLE-LOOP BREAKER` + `party=\[.venu`. **Measure elapsed:** `grep -oE '\[ *[0-9]+\.[0-9]+s\]' <log> | tail -1` (45-min run ≈ 2700s sim; <1000s = died early — inspect the tail).
  - **IF the Route-11 livelock is dead AND the bench climbs on Route 13-15 + she progresses past Fuchsia toward Saffron (badge 6)** → flip `POKEMON_BENCH_TO_MILESTONE` default "1" (~line 128; PREP_DRY_RESET + ACE_BAIL already 1) + commit. Team-depth wall broken → push badge 6 (Sabrina) → Silph → badge 7/8.
  - **IF the ACE-DOWN GUARD heal-THRASHES** (many `GRIND: ACE dinged` per tick, slow/circly on a watch) → raise `POKEMON_GRIND_ACE_BAIL_FRAC` lower (e.g. 0.25) so it fires later, OR add a per-tick heal cap. It's defaults-safe (`POKEMON_ACE_BAIL=0` reverts).
  - **IF the bench STILL stays thin** (PREP RE-ARM fires but bites small — participation switch soaks KO-XP into the ace, NS#5) → the missing lever is `SOLO_WEAK_GRIND` (campaign.py ~99/6347: give the bench the KILL XP). NS#10 productivity gate is the park-proof net; enable it for the to-milestone climb, verify no faint-thrash, re-run.
  - **IF the post-Koga Route-15 stall persists even with a leveled bench** → NAV/gate issue (route Fuchsia→Saffron via cycling road / Lavender, not back through the Route-15 gauntlet) — diagnose head_to_gym routing from Fuchsia.
  **RE-RUN cmd** (FRESH log always): `POKEMON_ACE_BAIL=1 POKEMON_BENCH_TO_MILESTONE=1 POKEMON_PREP_DRY_RESET=1 LONGRUN_BATTLE_LOG=1 POKEMON_KEEPER_STATIC_ROUTE=1 POKEMON_KEEPER_ROUTER=1 ../.venv/Scripts/python.exe -u recon_longrun.py surge_done_kit.state 45 > /g/temp/longrun/ns11_acebail2.log 2>&1 &`. Decision checks: `POKEMON_BENCH_TO_MILESTONE=1 ../.venv/Scripts/python.exe -u recon_bench_milestone_check.py` (12/12) + `../.venv/Scripts/python.exe -u recon_lopsided_grind_check.py`.

### ⚠️ NB — the deep WHITE-BOX SWITCH wedge is the ultimate root but flagged DON'T-FIX-BLIND (E4-livelock family, needs rule-15 live frame-grabs). The ACE-DOWN GUARD is the tractable upstream fix: keep the ace ALIVE so grass stays winnable and the wedge never gets a chance to trigger. If a future run STILL shows the white-box switch failing on a legit mid-battle switch (not caused by an over-worn team), THAT needs the frame-grab session.
