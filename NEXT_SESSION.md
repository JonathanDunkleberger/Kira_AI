...
## ⏳ NIGHT-SHIFT #11 IN FLIGHT (2026-07-11, night_shift.ps1 shift 11) — BUILT the autonomous Blaine/Safari Surf-prereq wiring (the NS#10 frontier): extracted `safari_strike.py`, registered it, wired PROACTIVE recognition — all decision-verified 8/8 + the recognition PROVEN to fire LIVE. The live look-ahead then fingered + I FIXED a REAL general routing bug (Poké-Flute key-item detection). TWO commits (`b52e34e` safari wiring flag-OFF; `e45e693` flute-routing fix). RE-VERIFY look-ahead IN FLIGHT. START HERE.
...
2. **`e45e693` — REAL general routing bug the live look-ahead fingered (independent of the flag).** The first look-ahead drove her the WRONG way (north to Bill) because the surf questline's anchor-route to Fuchsia (3,7) returned None → fell to the KB's local `dir="north"`. ROOT: `_story_gate_avoid` gated the Route-12/16 (Snorlax) avoid on `_item_count(350)>0`, but the Poké Flute (350) is a **KEY item** and `_item_count`/`bag_count` only scan the ITEMS pocket (0x310) — so the check NEVER fired and Route 12/16 stayed avoided even POST-Flute, blocking the ONLY learned graph path south to Fuchsia. FIX: read the KEY-ITEMS pocket (0x3B8). **VERIFIED** on koga_done_kit (Flute + Snorlax woken 0x253): `story_gate_avoid` now empties, `_next_step_rideable(→Fuchsia)` returns a valid hop east toward the southern road (was None). Pre-Flute behavior unchanged.

### ⇒ SHIFT-11 FRONTIER (priority order):
1. **READ the re-verify look-ahead (`/g/temp/longrun/ns11_safari_verify2.log`, `POKEMON_SAFARI_STRIKE=1` + koga_done_kit).** With the flute fix, she should now route SOUTH/east (Route 9→Route 10→Rock Tunnel→Lavender→Route 12→13→Fuchsia), the Safari strike fires at Fuchsia, she gets Surf (+Strength), the teach bridge teaches Surf, and the sea to Cinnabar opens. **GREP:** `🌊 PROACTIVE|QUESTLINE STRIKE.*Safari|got_surf|SAFARI DONE|STATE IN: Fuchsia|STATE IN: Cinnabar|TEACH BRIDGE.*surf|Volcano`. If she reaches Fuchsia + the strike fires + gets Surf cleanly → **FLIP `POKEMON_SAFARI_STRIKE` default ON (campaign.py ~195) + commit** (the last step). If a NEW blocker on the route to Fuchsia OR inside the Safari (the tour's closures are proven but the LIVE bridge exercises new paths) → diagnose the tail, fix, re-run.
2. **After Surf lands: the follow-on chain** — Cinnabar sea-nav → Secret Key/Mansion strike (recon_mansion.py exists, unpromoted, same extract-and-register pattern) → `GYM_PREREQS["Blaine"]` door → Blaine → Giovanni. Victory Road later needs Strength (already obtained by the same Safari strike) + Surf.
3. **Carry-overs (unchanged, lower pri):** PP-famine flee-spam in the lopsided grind stint (NS#10 2nd finding); grind-spot lever a (Route-23 nav-blocked, flag OFF); `POKEMON_BENCH_TO_MILESTONE` STAYS default-OFF.

**RE-RUN cmds:** Safari verify = `POKEMON_SAFARI_STRIKE=1 POKEMON_KEEPER_STATIC_ROUTE=1 POKEMON_KEEPER_ROUTER=1 LONGRUN_BATTLE_LOG=1 ../.venv/Scripts/python.exe -u recon_longrun.py koga_done_kit.state 50`. Decision check: `../.venv/Scripts/python.exe recon_safari_gate_check.py` (8/8). Canonical Champion save UNTOUCHED (all look-aheads on scratch koga_done_kit).
