...
**THE ROOT (precise):** Route 18 wilds L23-29 ≈ the L26 bench → near-0 XP/battle → floor can't reach the +6 pin (L32). The over-level release guards fire only on grass-**UNREACHABLE** (`_prep_dry>=2`) or grass-**dry** (`no_safe_grass`) — but Route 18 grass IS reachable and battles DO fire, so neither guard trips; the missing guard is a **grind-INADEQUACY release** (N grind stints with ZERO floor gain → release + cross, or route to a higher spot). That's the flagged level-aware grind-spot picker (`POKEMON_GRIND_SPOT_LEVELAWARE`, held OFF, NS#5/#7) + the earlier-whole-climb-leveling root (`TEAM_DEPTH_ROOT_FIX.md`).
**WHAT I DID:** glanced + quantified the treadmill (WAR order 4). **NO code change** — the fix is a grind-cadence/over-level edit = HARD-RULE-forbidden to blind-ship at night (@CLAUDE.md ~L405 no-shortcuts), and the emulator is busy (SINGLE-RUN LAW) so nothing could be verified anyway; ~$61 budget. Left the detached run alone (killing/relaunching = same code = same treadmill; banked_LIVE holds the furthest clean point). NO commit of code; committed this sharpened diagnosis as proof-of-life.
**⚠️ MONDAY (verify-gated, the fix): do NOT expect a resume of `banked_LIVE` to self-cross — it IS this treadmill state.** The clean fix = add a **grind-inadequacy poor-spot-release** (release the over-level after K stints with 0 floor gain → cross underleveled, OR lower the sea-leg over-level bite pin so she crosses at ~L29 instead of chasing L32 she can't reach), THEN boot a **corrected fresh run** (per shift-27's "delete banked_LIVE + fresh run"). Deeper/better = the fresh-GO whole-climb organic leveling so she arrives at Fuchsia already ~L36-40 (NS#6/#7). NB even after crossing, Blaine on grass-less Cinnabar is a coin-flip (NS#4) and the E4 is a separate team-depth wall (NS#22) — this treadmill fix only unblocks the Fuchsia→Cinnabar leg.
