...
1. **READ the full-length grind look-ahead result, then FLIP `POKEMON_CAVE_GRIND` default ON if clean.** A full-length run is IN FLIGHT at shift close: `POKEMON_CAVE_GRIND=1 GRIND_TARGET=55 VR_BUDGET_S=1500 POKEMON_GRIND_WEAK_PROBE_S=150 POKEMON_GRIND_WEAK_BUDGET_S=200 ../.venv/Scripts/python.exe -u recon_vr_grind_smoke.py` → `/g/temp/longrun/ns17_vrgrind_full.log` (6 stints, target 55). **AT RESUME grep** `RESULT:|SMOKE|VR-GRIND: (stint|done|bench floor)|routing south to Viridian|STRENGTH time` — confirm: the bench climbs steadily (no park/livelock), 0 Viridian abandons, 0 boulder shoves, returns to Indigo. ⚠️ **KEY NUANCE the run answers: VR2F wilds are L36-46, so the bench likely CAPS at ~L46-50 (XP falls off past the wild band), NOT the full E4 milestone L55.** That is still a big improvement (thin L39 → ~L46-50 + Lapras Ice Beam @L43), but may leave a RESIDUAL E4 gap (the NS#1 champion clear used L72 Lapras). If the run is clean → **set `CAVE_GRIND_ENABLED` default "1"** (campaign.py:125, one char) + commit; the endgame team-depth lever is armed. If it parks/wedges → keep OFF + diagnose (the loop is park-guarded, so a park would be a new edge case).
