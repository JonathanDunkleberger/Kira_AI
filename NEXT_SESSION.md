...
## ⏳ NIGHT-SHIFT #5 IN FLIGHT (2026-07-12, night_shift.ps1 shift 5) — BUILT the NS#4-frontier-#1 fix: the OVER-LEVEL-BEFORE-A-GRASS-LESS-SEA-LEG lever (flag `POKEMON_OVERLEVEL_SEALEG`, DEFAULT OFF), decision-verified 12/12, canonical UNTOUCHED. ONE commit (`d5f0fcc`). Look-ahead verification IN FLIGHT. START HERE ↓.

### ✅ WHAT NS#5 BUILT (`d5f0fcc`, campaign.py + recon_overlevel_sealeg_check.py, flag DEFAULT OFF, canonical UNTOUCHED):
The NS#4 wall: an evened badge-6 team STALLS at Blaine — it crosses to the grass-LESS Cinnabar island underleveled and a lost gym has NO recovery grass → whiteout-loop STALL. **THE FIX (fix #1 from the NS#4 frontier, park-safe):** at the LAST reachable grass before the crossing (Fuchsia/Route 18), keep the bench-to-milestone climb going AND DEFER the seafoam crossing questline while that climb is still productive, so she over-levels the bench THEN crosses.
- **`_overlevel_before_sealeg(state)`** — returns the Blaine milestone iff she's on the near side of the crossing (next_gym==Blaine, `_seafoam_gate()` open = has-Surf-not-crossed) with the bench under milestone. **Park-safe by construction** — releases (returns None) the instant ANY of: bench within `BENCH_MS_CLOSE` of milestone (leveled), this grass proved a poor spot (`_bench_poor_maps`, bench out-levelled it), or grass unreachable (`_prep_dry>=2` stand-down). So it can NEVER infinitely park (worst case she crosses with whatever the last grass gave, still far better than the L25-29 catch-level bench that stalls today). Scoped to the ONE `GRASSLESS_CROSSING_GYMS={"Blaine"}` transition (byte-unaffected elsewhere).
- **`_bench_to_ms_active(state)`** = `BENCH_TO_MILESTONE or (overlevel pending)` — locally enables the PROVEN NS#10 keep-climb machinery (climb-in-+6-bites + poor-map release) for this transition. Wired into 3 read-sites: the prep-pin keep-climb (`_prep_team_target`), the LOPSIDED-BENCH poor-map gate, and the grind executor's productivity gate. `_ensure_forward_questline` defers the seafoam-prereq recognition while pending.
- **THREE-STATE:** COMPILES (py_compile). WIRED (3 keep-climb sites + the questline defer). DECISION-VERIFIED 12/12 (`recon_overlevel_sealeg_check.py`: fires at Blaine near-side under-milestone; byte-inert OFF; releases on leveled/poor-map/prep-dry/crossed/non-Blaine/thin/post-game). Existing bench/milestone/lopsided/solo/grind-spot checks all GREEN (byte-inert when flag OFF: `_bench_to_ms_active==BENCH_TO_MILESTONE`).

### ⏳ THE FLIP GATE (the look-ahead IN FLIGHT — read it AT RESUME): `POKEMON_OVERLEVEL_SEALEG=1 POKEMON_LOPSIDED_GRIND=1 POKEMON_KEEPER_STATIC_ROUTE=1 POKEMON_KEEPER_ROUTER=1 LONGRUN_BATTLE_LOG=1 LONGRUN_BANK_EVERY_S=120 ../.venv/Scripts/python.exe -u recon_longrun.py fuchsia_evened_kit.state 90` → `/g/temp/longrun/ns5_overlevel.log`. **THE QUESTIONS:** (1) does she GRIND at Route 18 before the seafoam crossing (grep `OVER-LEVEL BEFORE SEA LEG|LOPSIDED-BENCH|BENCH-TO-MS: productive|floor crossed`)? (2) does the bench floor climb meaningfully above L25-29 (grep `party=\[` / `recon_partydump` the banked_LIVE)? (3) does she THEN cross (grep `reached_cinnabar|SEAFOAM|MANSION|Blaine`) and CLEAR Blaine (grep `Volcano|badges=7`)? (4) **NO PARK** — she must cross eventually (poor-map release fires when Route 18 caps ~L35), NOT grind Route 18 forever (grep for the last elapsed `[Ns]` + whether MAP TRANSITIONs continue). If clean (over-levels ~L25→L35+ then crosses + clears/attempts Blaine, no park) → **flip `POKEMON_OVERLEVEL_SEALEG` default "0"→"1" (campaign.py, one char) + commit.** If it PARKS at Route 18 → keep OFF, diagnose (maybe the poor-map release needs a stint budget backstop) + document. ⚠️ Process-death is common (Jonny's desktop load) — `banked_LIVE` holds the furthest point; resume from it.

### ⇒ NS#5 FRONTIER (priority order):
1. **READ the in-flight look-ahead + FLIP the lever if clean** (above). This is the shift's build; the flip-gate is the verify.
2. **COMPOUND with the type-answer-lead lever (NS#4 fix #3b) IF Blaine still walls even over-leveled.** The Blaine loss fielded Venusaur (fire-WEAK) 31× while Lapras (Water 2x) was NEVER LED. Even an L35 Lapras leading vs Blaine's L40-47 fire (Water resists Fire + Surf 2x) + heals likely wins where a fire-weak L57 Venusaur solo can't. ⚠️ Battle-AI = wedge-prone core (E4-livelock family) — DON'T-FIX-BLIND; the DECISION change (lead the SE type-answer vs a gym) is offline type-math but adds switch/menu exposure → careful + decision-verify.
3. **WIDEN E4_PREP_BAND 25→30 (downstream, only bites at VR2F)** — env-tunable today (`POKEMON_E4_PREP_BAND=30`); a realistic evened L28-29 bench isn't floored as box-fodder at VR2F. Low-risk.
4. **NB VR2F still caps ~L46 < L55 (NS#22 firm)** — even solving Blaine, the endgame needs a longer/higher grind to reach the E4 floor. Full chain: over-level at last grass (#1, this shift) → survivable Blaine/Giovanni → VR2F top-up → still need earlier over-level or a better endgame grind spot for the champion.

### RE-RUN cmds: decision check = `../.venv/Scripts/python.exe recon_overlevel_sealeg_check.py` (12/12). Flag OFF revert = default (byte-inert). fuchsia_evened_kit rebuild = `SLOTS=1,2,3,4 DONOR=koga_done_kit DEST=surf_ready_kit OUT=fuchsia_evened_kit ../.venv/Scripts/python.exe recon_transplant_party.py`.
