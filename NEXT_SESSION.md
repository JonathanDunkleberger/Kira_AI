...
## ⏳ NIGHT-SHIFT #4 (2026-07-12, night_shift.ps1 shift 4) IN FLIGHT — PIVOT: skip the slow fixture-build grind; run the mission-gate look-ahead END-TO-END from a CLEAN-NAV thin fixture. START HERE ↓.

### 🎯 THE PIVOT (why): the NS#27 fixture-build grind (recon_grind_bench on surf_ready_kit's thin bench) is TOO SLOW/UNEVEN to produce fuchsia_evened_kit — `banked_GRIND` after the run = Spearow L13→19 only; **Rattata L8 / Abra L10 / Drowzee L13 barely moved** (Route 18 wilds L23-29 are too high for the L8 mons → tiny participation share; and recon_grind_bench grinds species[0]→target before cycling, so it never reached Rattata/Abra/Drowzee). Building an evened fixture this way would take hours. **INSTEAD:** run the endgame look-ahead DIRECTLY from `surf_ready_kit` (AT Fuchsia, badge 6, THIN bench L8-25 + Venusaur L57, NAVS CLEANLY past the Route-9 gap — NS#27 verified MAP TRANSITION Fuchsia→Route18). This is the ACTUAL mission gate (does the armed machinery — selective-solo default-ON + cave-grind default-ON + participation switch — take a thin bench to a leveled-6 by Indigo?) AND it's unblocked (clean nav, right chain: Fuchsia→Seafoam→Cinnabar→Blaine→Giovanni→VR→E4, all wired+armed NS#13-17).

### ✅ surf_ready_kit RESULT (thin-bench data point, DONE): reached **Victory Road at badge 8 in ~289s, ALL CLEAN** (Blaine badge7 @203s lost-once-retried-won, Giovanni badge8 @289s, head_to_league→VR, beat Gary at the VR gate) — endgame nav+dispatch works end-to-end. **BUT the bench NEVER leveled**: `[venusaur 61, spearow 13, rattata 8, abra 10, drowzee 13, lapras 25]` unchanged the whole way. ROOT: the badge 6→8 **sea-island stretch (Fuchsia→Cinnabar→Route 20/21) has ZERO grindable grass** → LOPSIDED-BENCH + mid-game prep both abort `no_safe_grass`; and E4-prep floors sub-L30 members as box-fodder chaff (E4_PREP_BAND=25 → floor L30), so surf_ready's L8-25 bench is ALL below the floor → never fielded. **This is a FIXTURE ARTIFACT** (surf_ready's bench is unrealistically thin — a real fresh-GO arrives at badge 6 with an evened ~L28-34 bench). So surf_ready is NOT a fair test of the endgame machinery.

### ⏳ THE REAL TEST (running this shift) — built `fuchsia_evened_kit` via PARTY TRANSPLANT (`recon_transplant_party.py`, NEW): koga_done_kit's EVENED team (Venusaur L62 + Raticate/Dugtrio/Kadabra/Pidgeotto/Fearow L28-29) written into surf_ready_kit's Fuchsia RAM position + world_model (koga's journey/soul/strat sidecars). Byte-perfect (moves decrypt correctly → checksums survived; same OT=Kira → obeys). This is "a fresh-GO player who just beat Koga, standing at Fuchsia, evened team" — game-consistent, CLEAN-NAV (surf_ready's position, no Route-9 wedge), RIGHT team-shape. THE RUN:
`cd pokemon_agent && POKEMON_KEEPER_STATIC_ROUTE=1 POKEMON_KEEPER_ROUTER=1 LONGRUN_BATTLE_LOG=1 LONGRUN_BANK_EVERY_S=120 ../.venv/Scripts/python.exe -u recon_longrun.py fuchsia_evened_kit.state 120` → `/g/temp/longrun/ns4_evened_endgame.log`. Solo + cave-grind DEFAULT ON. **THE OPEN QUESTION (frontier #2, NS#26): does armed-selective-solo + VR2F cave-grind push the EVENED bench (L28-29) past ~L46 (VR2F participation-share cap, NS#22) toward the E4 floor L55 at Indigo?** `recon_partydump G:/temp/longrun/banked_LIVE/kira_campaign.state` at the VR/Indigo bank. Leveled-6 ~L50+ at Indigo → machinery works → reliability battery. Still ~L46 → VR2F genuinely insufficient (NS#22 firm), need longer VR grind / earlier over-level / better endgame grind spot.

### ✅ WHAT STANDS: selective-solo ARMED (default ON, `e2341f7`) + 17/17. PP-heal + load-share OFF. Canonical UNTOUCHED. `banked_GRIND` + surf_ready endgame log = scratch. NEW tool `recon_transplant_party.py` (party-donor → position-donor fixture builder) — commit it.
