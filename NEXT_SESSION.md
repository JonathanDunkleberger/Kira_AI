...
## ⏳ NIGHT-SHIFT #24 IN FLIGHT (2026-07-11, night_shift.ps1 shift 24) — BUILT resumable mid-run banking (kills the NS#18/19/21/23 process-death blocker that gates the fresh-GO leveled-6 verification), then RE-LAUNCHED the fresh-GO endgame diagnostic RESUMABLY. START HERE → the frontier is unchanged: THE FRESH-GO LEVELED-6 lever (give the bench kill-XP), but now the diagnostic that verifies it can survive process-death.
...
- Parses OK. **VERIFY** (in flight): the relaunched diagnostic below must produce `banked_LIVE` within ~2 min + round-trip clean → then COMMIT recon_longrun.py.
...
2. **USE the now-resumable diagnostic to answer the Indigo-arrival question** (above) — the data that gates whether/how much lever #1 is needed.
3. **The load-share flag STAYS OFF** (NS#23) — a verified building block for a fresh-GO team WITH two overlapping SE attackers.
