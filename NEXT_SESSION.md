...
## ⏳ NIGHT-SHIFT #11 IN FLIGHT (2026-07-11) — the ns10 run got FURTHER than predicted (BEAT KOGA), exposing the TRUE root: a `_prep_dry` DEADLOCK. FIX BUILT + COMMITTED `4a4387d`. Verification run in flight. START HERE.

### ✅ THE ns10_bench_ms VERDICT (read the whole run — it OVER-delivered): with `POKEMON_BENCH_TO_MILESTONE=1`, a fresh surge_done_kit climbed badge-3 Vermilion → **BADGE 4 (Erika)** → Rocket Hideout/Silph Scope → Pokémon Tower → **Poké Flute → Snorlax** → Route 12/13/14/15 → **BEAT KOGA → *** SOUL BADGE *** (badge 5)** — further than NS#10 predicted. She brute-forced Koga with the over-leveled **Venusaur L53** despite a thin bench. THEN she **STALLED post-Koga at Fuchsia** (`OUTCOME: STALL` @ (3,7)@(47,21), 14 no-progress decisions): her frozen **L15-20 bench** can't survive the **gated Route-15 trainers** (a doduo full-team keeps blacking her out) to route north toward **Saffron/Sabrina (gym 6)** → heal↔head_to_gym ping-pong. Final party `venusaur L53, rattata L16, mr.mime L20, pidgey L16, diglett L19, spearow L15`.

### 🎯 THE ROOT (this is the real find — a hard DEADLOCK, not a tuning gap): **PREP STAND-DOWN never clears.** The `_prep_dry >= 2` stand-down drops the 'train first' plan when grind attempts from a position find no grass. Its comment claims it "resets the moment any grass is reached" — but the ONLY reset (grind_weak_members returning ready/ok, campaign.py ~10790) sits INSIDE the grind path that `_prep_dry>=2` itself gates OFF (it gates BOTH the +6 prep pin @6207 AND the lopsided/bench-to-ms dominance @6538). So a single dry spell (grassless Celadon, PREP STAND-DOWN fired ONCE @ line 24479) **latched `_prep_dry`=2 for the ENTIRE rest of the run** → she marched the whole Koga chain PAST the level-appropriate **Route 13-15 grass (L23-33)** without ever stopping to grind → thin bench at Koga AND the post-Koga Route-15 wall. **The bench-to-milestone lever (b17de82) was correct but NEUTERED by this deadlock — it fired at badge 3 (bench 8→14) then never again.**

### ✅ FIX BUILT + COMMITTED `4a4387d` (flag `POKEMON_PREP_DRY_RESET` default ON, one-line revert; mode-side, canonical UNTOUCHED): at the roam tick top, clear `_prep_dry` the moment she stands on GENUINELY-USABLE new grass — `_reachable_grass()` here AND this map not in `_grind_dead`. Re-arms the bench-leveling machinery on the Route 13-15 grass she marches past. **Spin/shuttle-proof by construction:** grassless towns + ledge-gated grass → `_reachable_grass` None → no reset (no Fuchsia↔Route-15 shuttle, the run6 hazard); fragile one-way-strand / dead maps → in `_grind_dead` → no same-map spin. **`recon_bench_milestone_check` 12/12 UNREGRESSED.**

### ⏳ VERIFICATION RUN IN FLIGHT (the ship-gate — READ ITS TAIL AT RESUME): `G:/temp/longrun/ns11_prepdry2.log`, fresh surge_done_kit 45-min, `POKEMON_BENCH_TO_MILESTONE=1 POKEMON_PREP_DRY_RESET=1` (launched NS#9-this-shift ~13:xx). **NB the earlier `ns11_prepdry.log` was KILLED PREMATURELY at ~70s wall** (the prev session's process tree died at its context-wall handoff before the 45-min cap) — it only reached "caught diglett + started LOPSIDED grind at Vermilion", NEVER marched the Koga chain, so it says NOTHING about PREP RE-ARM. Ignore it; read `ns11_prepdry2.log`. (That killed run DID reproduce a real watchability datum: the barren Diglett's-Cave vestibule (1,38) burns its full 45s `KEEPER_CAVE_FLOOR_WANDER_S` budget wandering an encounter-less room before descending — bounded, not a livelock, but ~10 game-min of dead air. Low-pri fix = step-count barren detector, NS#41/#42-flagged.) THE QUESTION: does she now **STOP on Route 13/14/15 and grind the bench** (PREP RE-ARM fires; LOPSIDED-DBG re-prints post-badge-4 with the gaps), arrive at Koga with a **leveled bench**, and **progress PAST the post-Koga Route-15 stall toward Saffron**? **AT RESUME grep:** `PREP RE-ARM|PREP STAND-DOWN|LOPSIDED-DBG|BENCH-TO-MS|Soul Badge|badges=5|badges=6|OUTCOME:|STALL` + `party=\[.venu`.
  - **IF the bench climbs on Route 13-15 + she progresses past Fuchsia toward Saffron (badge 6)** → flip BOTH `POKEMON_BENCH_TO_MILESTONE` AND `POKEMON_PREP_DRY_RESET` default "1" (BENCH_TO_MILESTONE ~line 128; PREP_DRY_RESET already 1) + commit. The Koga/team-depth wall is broken → push badge 6 (Sabrina) → Silph → badge 7/8.
  - **IF the bench STILL stays thin** (PREP RE-ARM fires but bites are small — the participation switch soaks KO-XP into the ace, the NS#5 mechanism) → the missing lever is (b) `SOLO_WEAK_GRIND` (campaign.py ~99/6347: give the bench the KILL XP, not just participation). The NS#10 productivity gate is the exact park-proof safety net its own comment (~88-100) asked for — enable it for the to-milestone climb, verify no faint-thrash, re-run. **This is the precise next lever if RE-ARM alone is insufficient.**
  - **IF the post-Koga Route-15 stall persists even with a leveled bench** → it's a NAV/gate issue (route Fuchsia→Saffron via the cycling road / Lavender, not back through the Route-15 gauntlet) — diagnose the head_to_gym routing from Fuchsia.
  **RE-RUN cmd** (output to a FRESH log; a killed run is worse than useless): `POKEMON_BENCH_TO_MILESTONE=1 POKEMON_PREP_DRY_RESET=1 LONGRUN_BATTLE_LOG=1 POKEMON_KEEPER_STATIC_ROUTE=1 POKEMON_KEEPER_ROUTER=1 ../.venv/Scripts/python.exe -u recon_longrun.py surge_done_kit.state 45 > /g/temp/longrun/ns11_prepdry3.log 2>&1 &`. Decision check: `POKEMON_BENCH_TO_MILESTONE=1 ../.venv/Scripts/python.exe -u recon_bench_milestone_check.py` (12/12).
