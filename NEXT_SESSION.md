...
1. **CONFIRM the look-ahead** (`/g/temp/longrun/ns14_giovanni_dispatch.log`, from `blaine_done_kit` with ALL 5 flags on: `POKEMON_GIOVANNI_GYM=1 POKEMON_BLAINE_GYM=1 POKEMON_MANSION_STRIKE=1 POKEMON_SAFARI_STRIKE=1 POKEMON_SEAFOAM_STRIKE=1 POKEMON_LOPSIDED_GRIND=0 POKEMON_KEEPER_STATIC_ROUTE=1 POKEMON_KEEPER_ROUTER=1 LONGRUN_BATTLE_LOG=1 ../.venv/Scripts/python.exe -u recon_longrun.py blaine_done_kit.state 25`). grep `PROACTIVE EARTH-BADGE|QUESTLINE STRIKE.*Viridian|EDGE north|VIRIDIAN reached|GIOVANNI ->|EARTH BADGE|badges=8|Earth Badge`. SUCCESS = she opens the errand, the strike drives the 5 sea crossings to Viridian, beats Giovanni, badge 8. If it PARKS/wedges → diagnose the tail (the strike's cross_edge is smoke-proven from Cinnabar, so a failure would be in the dispatch/anchor wiring, not the road).
2. **THEN flip `POKEMON_GIOVANNI_GYM` default ON (campaign.py ~232, `"0"`→`"1"`) + COMMIT** all of shift 14 (giovanni_gym.py anchors, the JSON cap, the 4 campaign.py edits). Giovanni smoke still proves the strike in isolation: `GIOVANNI_STATE=blaine_done_kit ../.venv/Scripts/python.exe -u recon_giovanni_smoke.py` (grep `SMOKE PASS|EARTH BADGE`).
3. **THEN: Victory Road → E4 → CREDITS.** After badge 8 she heads to Route 22 (Gary ambush arms there — the giovanni strike does NOT drain it, it's the next objective's problem) → Victory Road (needs Strength [banked by Safari] + Surf). `recon_victory.py` + `recon_e4.py` exist (champion tail scripts, unwired) — SAME extract-and-hook pattern: VR = boulder/ladder nav → a `victory_road` questline/travel strike (it's not a gym); E4 = the gauntlet (battle-brain already proven on the champion climb → an e4 strike). The autonomous fresh-GO chain to credits is a few more of these gates.
