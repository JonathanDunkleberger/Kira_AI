...
**VERIFIED (isolated, `recon_hm_boxswap_check.py`, ALL PASS):** boot banked_LIVE → `_box_swap_for_hm('surf')` → boxed a diglett 6→5, withdrew lapras 5→6, party now has a surf-learner, `default_plan(surf)` → real plan `(slot5, forget move54 pw0)`. The teach itself (TeachFlow) + Safari + sea-crossing are all separately proven (NS#11) — so the whole Blaine chain is unblocked. FULL e2e (cross Blaine) = the detached run's job (glance next shift).
**STATE:** killed the doomed reboot-loop + watchdog, relaunched the watchdog detached (PID 136193) booting banked_LIVE (badge 5, Sabrina, Lapras in box) with the FIXED code. It re-climbs Sabrina→badge6→Blaine→**box-swap fires**→Surf→Cinnabar→Blaine(7)→Giovanni(8)→VR→E4. Canonical saves UNTOUCHED.
