...
### ⇒ SHIFT-11 FRONTIER — the SEA-NAV to Cinnabar (the gating step before the flag flip), priority order:
1. **BUILD the Fuchsia→Cinnabar SEA-NAV (the one thing between the proven wiring and flipping the flag).** Post-Surf she soft-oscillates Fuchsia (3,7)↔Route 15 (3,33) instead of crossing to Cinnabar (3,8). ROOT (diagnosed): the billed road (`frlg_gates.json roads['Cinnabar Island']`) IS defined (Fuchsia `go:south` → Route 19 SEA → Route 20 → Cinnabar), BUT **Route 19 (3,37) is an UNVISITED map absent from the learned graph**, so head_to_gym can't route the south leg + falls back to the nearest VISITED anchor (Route 15 east) → the bounce. Same class as the NS#40 keeper static-connection gap. THE FIX: static-connection-aware routing to Cinnabar (seed the Fuchsia→Route19→Route20→Cinnabar sea chain from the disasm MapConnection graph, like `_host_gateways`/`gamedata/frlg_connections.json`) + surf-cross the sea legs (travel is already `surf-capable` once taught — logs confirm). VERIFY a look-ahead from a GOOD-bench badge-6 fixture (koga_done_kit once its Route-9 boot-pocket is escaped, OR a fresh clean fixture — NB fuchsia_lapras_kit has a THIN L8-13 bench that also triggers a lopsided-grind oscillation, confounding the read) reaches Cinnabar. NB the koga_done_kit boot at Route 9 (10,3) is a grind-POCKET that can't reach the Route-10 east exit — a fixture artifact; a fresh run beats Koga AT Fuchsia so the strike fires there (no Route-9 crossing).
2. **THEN FLIP `POKEMON_SAFARI_STRIKE` default ON (campaign.py ~197) + commit** — once a clean e2e reaches Cinnabar post-Surf with no soft-livelock. The wiring is BUILT + proven-to-Surf; only the sea-nav gates the flip.
