...
FRONTIER NOW (shift 4, 06:52): fresh_go_2 RELAUNCHED FRESH after a dup root-fix. GLANCE cheap, exit fast. On banked_CREDITS -> order 6 (run_stats -> RUN_STATS_fresh_go_2.md -> evaluate QUALIFYING -> CREDITS/WATCHABILITY-GAPS line). Hard wedge (whole-log spin, 0 progress many iters) -> ROOT-fix + resume. Healthy -> exit. If ace still over-climbs / bench < L42 at E4 (qualifying fail) -> the deferred order-3b `_road_bench_xp_arm` questline-guard relax is the next build (see DEFERRED above). WATCH the FINAL six are DISTINCT (dup fix below should hold, but verify at credits).
